...
description: Clean up after a PR merge, for plain branches and linked worktrees. Use when the user asks to clean up a merged branch or worktree, or after a PR merge.
---
...
3. If the current workspace is a linked worktree (`git rev-parse --git-common-dir` differs from `--git-dir`):
   ...
   - Move to the main worktree (`git worktree list --porcelain`), `git pull`, then `git worktree remove <worktree-path>` and `git branch -d <branch>`.
4. Otherwise (plain branch):
   - Stash if dirty (`git stash`), `git checkout main` → `git pull` → `git branch -d <branch>`, then `git stash pop` if stashed.
