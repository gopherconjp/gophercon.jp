---
description: "Use when writing or converting break-controlled copy for the Breakable component. Covers the pipe syntax (| || |||) for break priority, 改行位置指定, nested arrays, and *...* accent emphasis."
...
- Type `Breakable = string | Breakable[] | { accent: Breakable }` (`types.ts` in the same directory, also usable from `*.content.ts` via `t<Breakable>`). Unlimited nesting; outer breaks before inner. `{ accent }` renders in accent color, wraps any `Breakable`, and recurses; pipe-syntax parsing yields pipe-free string leaves.
...
- Invalid syntax (unmatched `*`, nested accent, pipe inside accent, empty accent): report an error naming which case occurred with the correct format.
...
1. Extract each non-greedy, pipe-free, non-empty `*...*` first as atomic `{ accent: <inner> }`.
