...
- Design files: `gopherconjp` (shared: `Core Components`, `404`) and `gopherconjp-2027` (year-specific). Confirm `penpot.currentFile.name` before editing; never assume which file is open.
...
- Band: board is text `+15`, text bottom at board `-15`, bottom-anchored (`constraintsVertical: bottom`); center variant centers both. Width via `leftright` / `left` / `center` per spec; board `auto` sizing, `clipContent: false` (flex children `layoutChild: fill`; `leftright` texts need `auto-height`).
...
- Text width follows content automatically (`auto-width`); never resize text-element height inside an instance (comes from the variant, e.g. `Lines`) — switch the variant or replace with a fresh instance instead.

## Verification

- `export_shape` PNG check; `penpot:export` plus snapshot commit, never skip.
